...
describe('IndivProjectComponent', () => {
  ...
  it('testing subscribe method is called',fakeAsync(() => {
    ...
    let eventDetailsSpy = spyOn(testBedService, 'getProjectDetails').and.returnValue(mockProject);
    let subSpy = spyOn(testBedService.getProjectDetails('4'), 'subscribe');
    component.ngOnInit();
    tick();
